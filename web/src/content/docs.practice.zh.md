...
想提前熟悉评测流程，在「参赛」页选「提交完整项目」，提交 GitHub 仓库或 ZIP。平台在云端逐轮运行你的程序（和比赛相同的评测流程），题目由练习赛数据生成。每队每天 5 次（因平台原因失败的不计次数），只能用本队自己的模型密钥，成绩进单独的完整项目榜。10 月 1–4 日培训期间建议用这种方式把流程走一遍。
