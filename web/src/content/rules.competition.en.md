...
10. **Random scenarios and sequential decisions.** In the current competition, each team receives one fixed private seed for weather, events and hidden tags: every evaluation of a team uses the same scenario, and different teams get different ones. Public observation tasks and scoring rules stay fixed. The platform records each decision before releasing the next observation; future rounds cannot be submitted early and accepted decisions cannot be replaced. A new evaluation attempt does not change the scenario.
